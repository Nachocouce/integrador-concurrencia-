...
class RepositorioEventos:
    ...
    def insertar_evento(
        self, nombre: str, fecha: str, lugar: str, precio: float, disponibles: int
    ) -> None:
        with sqlite3.connect(self.DB) as conn:
            cursor = conn.cursor()
            cursor.execute("""
...
class RepositorioVentas:
    ...
    def insertar_venta(
        self, evento_id: int, cliente: str, gmail: str, cantidad_boletos: int, total: float, fecha_venta: str
    ) -> None:
        with sqlite3.connect(self.DB) as conn:
            cursor = conn.cursor()
            cursor.execute("""
